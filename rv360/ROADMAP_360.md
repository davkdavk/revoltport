...
- 2026-09-15: **N0 observed complete.** `check_xenon.sh` now uses a content/header hash object cache and bounded parallel Xenon compiles; `build_xenon.sh` stages `build/rgh-current/Revolt/default.xex` and `rv360/shaders/*.xvu`. A cached 136-object release build completed in 28.6 seconds; build scripts, root `AGENTS.md`/`NEXT.md`, and `load.cpp` regression tests are now part of the repository worktree.
