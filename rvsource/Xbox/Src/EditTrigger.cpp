...
void InitFileTriggers(void)
{
    ...
    if (!FileTriggers)
    {
        DumpMessage(NULL, "Can't alloc memory for file triggers!");
        QuitGame();
    }
}
...
// load one file trigger
        ...
        VecPlusEqVec(&ftri.Pos, &EditOffset);

// setup edit trigger

        FileTriggers[i] = ftri;

        if (!TriggerNames[FileTriggers[i].ID])
        {
            FileTriggers[i].ID = 0;
            DumpMessage(NULL,"Dodgy trigger ID!");
        }
    }
...
// close file
    ...
    DumpMessage("Saved Trigger File:", file);
    fclose(fp);
}
...
// display name
        ...
        swprintf(buf, L"%S", TriggerNames[trigger->ID]);
...
// Flag
    ...
    {
        swprintf(buf, L"Flag low %d", trigger->Flag & 0xffff);
        DumpText(450, 24, 8, 16, 0x00ffff, buf);
    }
    else
    {
        swprintf(buf, L"Flag low %S", TriggerEnums[trigger->ID][trigger->Flag & 0xffff]);
        DumpText(450, 24, 8, 16, 0x00ffff, buf);
    }

    swprintf(buf, L"Flag high %d", trigger->Flag >> 16);
    DumpText(450, 48, 8, 16, 0xffff00, buf);

// axis

    swprintf(buf, L"Axis %S - %S", FileTriggerAxisNames[FileTriggerAxis], FileTriggerAxisTypeNames[FileTriggerAxisType]);
    DumpText(450, 72, 8, 16, 0xff00ff, buf);
}
...
// change flag low 16?
    ...
    flag = (short)(CurrentTrigger->Flag & 0xffff);
    ...
    CurrentTrigger->Flag = (CurrentTrigger->Flag & 0xffff0000) | flag;

// change flag high 16?

    flag = (short)(CurrentTrigger->Flag >> 16);

    if (Keys[DIK_LBRACKET] && !LastKeys[DIK_LBRACKET])
    {
        flag--;
    }
    if (Keys[DIK_RBRACKET] && !LastKeys[DIK_RBRACKET])
    {
        flag++;
    }
    ...
    CurrentTrigger->Flag = (CurrentTrigger->Flag & 0xffff) | (flag << 16);
