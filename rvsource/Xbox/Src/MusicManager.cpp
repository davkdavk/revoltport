...
typedef struct {
    WCHAR * szName;
    CHAR *  szFilename;
    DWORD   dwLength;  // in msec
} MM_SONG;

MM_SONG g_aGameSoundtrack[] =
{
//$MODIFIED(cprince) - Goodbye crappy piano music; hello phat Re-Volt beatz
    { L"A_02_JamJar",         "D:\\Music\\A_02_JamJar.wma",         324000 },
    { L"B_03_ToysForTheBoyz", "D:\\Music\\B_03_ToysForTheBoyz.wma", 294000 },
    { L"B_04_LiveWires",      "D:\\Music\\B_04_LiveWires.wma",      234000 },
    { L"B_05_CandyJumping",   "D:\\Music\\B_05_CandyJumping.wma",   247000 },
    { L"C_06_Flipped",        "D:\\Music\\C_06_Flipped.wma",        180000 },
    { L"C_07_Recharge",       "D:\\Music\\C_07_Recharge.wma",       209000 },
    { L"C_08_WheelzOfSteel",  "D:\\Music\\C_08_WheelzOfSteel.wma",  180000 },
    { L"C_09_LittleToyCarz",  "D:\\Music\\C_09_LittleToyCarz.wma",  239000 },
    { L"C_10_OverDriver",     "D:\\Music\\C_10_OverDriver.wma",     364000 },
    { L"C_11_OutOfControl",   "D:\\Music\\C_11_OutOfControl.wma",   213000 },
    { L"D_12_WestVolt",       "D:\\Music\\D_12_WestVolt.wma",       195000 },
  //{ L"E_13_WinCup",         "D:\\Music\\E_13_WinCup.wma",          23000 },
  //{ L"F_14_LoseCup",        "D:\\Music\\F_14_LoseCup.wma",         22000 },
    { L"G_15_Credits",        "D:\\Music\\G_15_Credits.wma",        121000 },
//$END_MODIFICATIONS
};

const DWORD NUM_GAME_SONGS = ( sizeof( g_aGameSoundtrack ) / sizeof( g_aGameSoundtrack[0] ) );

const DWORD WORKER_THREAD_STACK_SIZE = 12 * 1024;   // 12k stack for worker thread
const DWORD WMA_LOOKAHEAD = 64 * 1024;              // 64k lookahead buffer
const DWORD YIELD_RATE = 3;                         // yield rate of 3
...
//-----------------------------------------------------------------------------
HRESULT
CMusicManager::SetVolume( LONG lPercent )
{
    ...
    else 
    {
        ...
        m_lVolume = LONG( 2000 * log( lPercent / 100.0f ) );
    }

    m_pStream[0]->SetVolume( m_lVolume );
    m_pStream[1]->SetVolume( m_lVolume );
...
CMusicManager::DebugVerify()
{
    ...
    {
#ifdef _DEBUG
        OutputDebugString( _T("Must have at least 1 game soundtrack - music will be turned off.\n") );
#endif
        return E_FAIL;
    }
    ...
    for( UINT uSoundtrack = 0; uSoundtrack < m_uSoundtrackCount; uSoundtrack++ )
    {
        for( UINT uSong = 0; uSong < m_aSoundtracks[ uSoundtrack ].GetSongCount(); uSong++ )
        {
            ...
            {
#ifdef _DEBUG
                OutputDebugString( _T("Failed to open a soundtrack file - music will be turned off.\n") );
#endif
                return E_FAIL;
            }
            CloseHandle( h );
