/* $HACK: temporary file used while porting; will remove before release */
...
#define MSS_VERSION      "5.0a"
#define MSS_VERSION_DATE "17-Sep-98"

#define MSS_COPYRIGHT "Copyright (C) 1991-98 RAD Game Tools, Inc."

#endif

#ifndef MSS_H
#define MSS_H
#if 0  //$ADDITION - don't really want any Miles Sound System stuff
...
#define DEFAULT_DMC                24     // 24 allocatable SAMPLE structures
...
typedef S32        (AILCALL FAR * M3D_SET_3D_SAMPLE_FILE)(H3DSAMPLE samp,
                                                          void FAR *file_image);
...
typedef void (AILCALLBACK FAR* AILCODECSETPREF)(char FAR* preference,U32 value);
...
typedef struct _M3DPROVIDER
{
   ...
   M3D_SET_3D_SAMPLE_FILE        set_3D_sample_file;
...
#define /* S32 */ AIL_enumerate_3D_provider_attributes(/* HPROVIDER                 */ lib,  \
                                                       /* HINTENUM FAR *            */ next, \
                                                       /* RIB_INTERFACE_ENTRY FAR * */ dest) \
   RIB_enumerate_interface(lib,"MSS 3D audio services",RIB_ATTRIBUTE,next,dest)

#define /* void */ AIL_3D_provider_attribute(/* HPROVIDER  */ lib,  \
                                             /* C8 FAR *   */ name, \
                                             /* void FAR * */ val)  \
   { U32 token; *(S32 FAR *) val = -1; if (RIB_request_interface_entry(lib, "MSS 3D audio services",\
   RIB_ATTRIBUTE, name, &token) == RIB_NOERR) *(U32 FAR *) val = ((M3DPROVIDER FAR *) \
   RIB_provider_system_data(lib, 0))->PROVIDER_query_attribute(token); }

#define /* void */ AIL_set_3D_provider_preference(/* HPROVIDER */  lib,  \
                                                  /* C8 FAR *  */  name, \
                                                  /* void FAR * */ val)  \
   { U32 token; if (RIB_request_interface_entry(lib, "MSS 3D audio services",\
   RIB_PREFERENCE, name, &token) == RIB_NOERR) ((M3DPROVIDER FAR *) \
   RIB_provider_system_data(lib, 0))->set_provider_preference(token,*(U32 FAR *) val); }
...
DXDEC U32        AILCALL AIL_3D_sample_offset        (H3DSAMPLE     samp,
                                                      AILSNDPOSTYPE type);

DXDEC U32        AILCALL AIL_3D_sample_length        (H3DSAMPLE     samp,
                                                      AILSNDPOSTYPE type);
...
DXDEC H3DPOBJECT AILCALL AIL_3D_open_listener        (HPROVIDER lib);

DXDEC void       AILCALL AIL_3D_close_listener       (H3DPOBJECT listener);

DXDEC H3DPOBJECT AILCALL AIL_3D_open_object          (HPROVIDER lib);

DXDEC void       AILCALL AIL_3D_close_object         (H3DPOBJECT obj);
...
DXDEC  void     AILCALL AIL_set_3D_object_user_data  (H3DPOBJECT obj,
                                                      U32        index,
                                                      S32        value);

DXDEC  S32      AILCALL AIL_3D_object_user_data      (H3DPOBJECT obj,
                                                      U32        index);
//
...
#endif // 0  //$ADDITION - don't really want any Miles Sound System stuff
#endif
