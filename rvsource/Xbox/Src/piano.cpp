...
void TriggerPiano(PLAYER *player, long flag, long n, PLANE *planes)
{
...
// play
    ...
    SAMPLE_SFX *sample;
    sample = PlaySfx3D(SFX_TOY_PIANO, SFX_MAX_VOL, c * 800 + 12000, pos, 2);

// try to steal from older note if play failed

    if (!sample)
    {
        sample = GetOldestSfx(SFX_TOY_PIANO);
        if (sample)
        {
            StopSfx(sample);
            PlaySfx3D(SFX_TOY_PIANO, SFX_MAX_VOL, c * 800 + 12000, pos, 2);
        }
    }
 #else // !OLD_AUDIO
    // TODO (JHarding): Revisit with level-specific sounds.  Also implies we'll
    // need ability to pitch before playing
    g_SoundEngine.Play3DSound( g_dwLevelSoundsOffset + EFFECT_toy_piano, 
                               FALSE, 
                               pos->v[0], 
                               pos->v[1], 
                               pos->v[2] );
