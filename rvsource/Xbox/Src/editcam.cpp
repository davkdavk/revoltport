...
void InitEditCamNodes(void)
{
    ...
    if (!EditCamNode)
    {
        DumpMessage(NULL, "Can't alloc memory for edit camera nodes!");
        QuitGame();
    }
}
...
// load one file cam node
        ...
        fnode.x = (long)((float)fnode.x * EditScale + (EditOffset.v[X] * 65536.0f));
        fnode.y = (long)((float)fnode.y * EditScale + (EditOffset.v[Y] * 65536.0f));
        fnode.z = (long)((float)fnode.z * EditScale + (EditOffset.v[Z] * 65536.0f));
...
// setup edit cam node
        ...
        if ((fnode.RailType < 0) || (fnode.RailType >= CAM_RAILTYPE_NTYPES)) {
            fnode.RailType = CAM_RAILTYPE_RANDOM;
        }
        EditCamNode[i].RailType = fnode.RailType;
    }
...
// set file cam node
        ...
        fnode.RailType = EditCamNode[i].RailType;
...
// close file
    ...
    DumpMessage("Saved camera node File:", file);
    fclose(fp);
}
...
// draw ID
        ...
        swprintf(buf, L"%d", EditCamNode[i].ID);
...
// type
    ...
    swprintf(buf, L"Type %S", CamNodeTypeText[node->Type]);
    DumpText(450, 0, 8, 16, 0xff0000, buf);

// flag

    swprintf(buf, L"Zoom %d", node->ZoomFactor);
    DumpText(450, 24, 8, 16, 0x00ff00, buf);

// axis

    swprintf(buf, L"Axis %S - %S", EditCamNodeAxisNames[EditCamNodeAxis], EditCamNodeAxisTypeNames[EditCamNodeAxisType]);
    DumpText(450, 48, 8, 16, 0x0000ff, buf);

// pos

    swprintf(buf, L"Pos  %d %d %d", (long)node->Pos.v[X],    (long)node->Pos.v[Y], (long)node->Pos.v[Z]);
    DumpText(450, 72, 8, 16, 0xffffff, buf);

// ID

    swprintf(buf, L"ID   %d", node->ID);
    DumpText(450, 96, 8, 16, 0x00ff00, buf);

// RailCamType
    swprintf(buf, L"RailType: %S", RailTypeName[node->RailType]);
    DumpText(450, 120, 8, 16, 0x00ff00, buf);
...
// new node?
    ...
    if (Keys[DIK_INSERT] && !LastKeys[DIK_INSERT])
    {
        if ((node = AllocEditCamNode()))
        {
            ...
            node->RailType = CAM_RAILTYPE_RANDOM;
...
// change type?
    ...
    if (Keys[DIK_NUMPADENTER] && !LastKeys[DIK_NUMPADENTER]) {
        CurrentEditCamNode->Type++;
        if (CurrentEditCamNode->Type >= CAMNODE_NTYPES) {
            CurrentEditCamNode->Type = 0;
        }
    }
...
// change rail type
    if (Keys[DIK_MINUS] && !LastKeys[DIK_MINUS]) {
        CurrentEditCamNode->RailType--;
    }
    if (Keys[DIK_EQUALS] && !LastKeys[DIK_EQUALS]) {
        CurrentEditCamNode->RailType++;
    }
    Limit(CurrentEditCamNode->RailType, CAM_RAILTYPE_RANDOM, CAM_RAILTYPE_REVERSE);
...
// add link?
    ...
    if (Keys[DIK_NUMPADPLUS] && !LastKeys[DIK_NUMPADPLUS] && LastEditCamNode && LastEditCamNode != CurrentEditCamNode)
    {
        ...
        if ((CurrentEditCamNode->Type != CAMNODE_WATCHPOINT) && (LastEditCamNode->Type != CAMNODE_WATCHPOINT)) {
