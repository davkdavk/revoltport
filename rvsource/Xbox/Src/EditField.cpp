...
void InitFileFields(void)
{
    ...
    if (!FileFields)
    {
        DumpMessage(NULL, "Can't alloc memory for file fields!");
        QuitGame();
    }
}
...
// load one file field
        ...
        VecPlusEqVec(&ff.Pos, &EditOffset);
...
// close file
    ...
    DumpMessage("Saved Field File:", file);
    fclose(fp);
}
...
// display name
        ...
        swprintf(buf, L"%S", FileFieldTypeNames[field->Type]);
...
// axis
    ...
    swprintf(buf, L"Axis %S - %S", FileFieldAxisNames[FileFieldAxis], FileFieldAxisTypeNames[FileFieldAxisType]);
    DumpText(450, 24, 8, 16, 0xff00ff, buf);

// rot

    swprintf(buf, L"Rot %S", FileFieldRotNames[FileFieldRotType]);
...
// mag
    ...
    {
        if (EnterFlag && !Picker) 
			swprintf(buf, L"Mag %S_", EnterString);
        else 
			swprintf(buf, L"Mag %ld.%04ld", (long)field->Mag, (long)((field->Mag - (float)(long)field->Mag) * 10000));
        DumpText(450, 72, 8, 16, 0x00ffff, buf);
    }

// damping

    if (field->Type == FILE_FIELD_TYPE_LINEAR || field->Type == FILE_FIELD_TYPE_ORIENTATION || field->Type == FILE_FIELD_TYPE_WIND)
    {
        if (EnterFlag && Picker == 1) 
			swprintf(buf, L"Damping %S_", EnterString);
        else 
			swprintf(buf, L"Damping %ld.%04ld", (long)field->Damping, (long)((field->Damping - (float)(long)field->Damping) * 10000));
...
// grad start
    ...
    {
        if (EnterFlag && !Picker) 
			swprintf(buf, L"Grad Start %S_", EnterString);
        else 
			swprintf(buf, L"Grad Start %ld.%04ld", (long)field->GradStart, (long)((field->GradStart - (float)(long)field->GradStart) * 10000));
        DumpText(450, 72, 8, 16, 0x00ffff, buf);
    }
...
// grad end
    ...
    {
        if (EnterFlag && Picker == 1) 
			swprintf(buf, L"Grad End %S_", EnterString);
        else 
			swprintf(buf, L"Grad End %ld.%04ld", (long)field->GradEnd, (long)((field->GradEnd - (float)(long)field->GradEnd) * 10000));
        DumpText(450, 96, 8, 16, 0x00ff00, buf);
    }

// picker

    DumpText(426, Picker * 24 + 72, 8, 16, 0xffffff, L"->");
}
...
// save fields?
    ...
    {
        SaveFileFields(GetLevelFilename("fld", FILENAME_MAKE_BODY));
    }
...
//    LoadModel("edit\\field2.m", &FileFieldModel[1], -1, 1, LOADMODEL_FORCE_TPAGE, 100);
    LoadModel("D:\\edit\\field1.m", &FileFieldModel[0], -1, 1, LOADMODEL_FORCE_TPAGE, 100);
    LoadModel("D:\\edit\\field2.m", &FileFieldModel[1], -1, 1, LOADMODEL_FORCE_TPAGE, 100);
//$END_MODIFICATIONS
}
