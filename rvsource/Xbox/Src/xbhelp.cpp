...
static D3DXVECTOR2 g_vHelpCallouts[] = 
{
    ...
    D3DXVECTOR2( 440.0f, 156.0f ),
    D3DXVECTOR2( 445.0f, 130.0f ), D3DXVECTOR2( 447.0f, 118.0f ),
    D3DXVECTOR2( 445.0f, 130.0f ), D3DXVECTOR2( 447.0f, 118.0f ),
...
HRESULT CXBHelp::Create( CHAR* strResource )
{
    ...
    if( FAILED( m_xprResource.Create( strResource, 1 ) ) )
        return E_FAIL;
    ...
    D3DDevice_CreateVertexBuffer( 4*6*sizeof(FLOAT), D3DUSAGE_WRITEONLY, 
                         ...
                         DWORD dwNumCallouts, FLOAT fOffsetX, FLOAT fOffsetY )
{
    ...
    D3DDevice_SetTexture( 0, m_pGamepadTexture );
    ...
    D3DDevice_SetRenderState( D3DRS_FOGTABLEMODE, D3DFOG_NONE );
    ...
    D3DDevice_SetStreamSource( 0, m_pVB, 6*sizeof(FLOAT) );
    D3DDevice_DrawVertices( D3DPT_QUADLIST, 0, 4 );

    // Set state to draw the lines
    D3DDevice_SetTexture( 0, NULL );
    D3DDevice_SetRenderState( D3DRS_ALPHABLENDENABLE, FALSE );
    D3DDevice_SetVertexShader( D3DFVF_XYZRHW );
    ...
    for( DWORD i=0; i<dwNumCallouts; i++ )
    {
        ...
        D3DXVECTOR4 v[2];
        v[0] = D3DXVECTOR4( line1x + fOffsetX, line1y + fOffsetY, 0.0f, 0.0f );
        v[1] = D3DXVECTOR4( line2x + fOffsetX, line2y + fOffsetY, 0.0f, 0.0f );
        
        D3DDevice_DrawVerticesUP( D3DPT_LINELIST, 2, v, sizeof(D3DXVECTOR4) );
    }
    ...
    for( i=0; i<dwNumCallouts; i++ )
    {
        ...
        FLOAT textx = g_vHelpCallouts[wTextPosIndex].x + fOffsetX;
        FLOAT texty = g_vHelpCallouts[wTextPosIndex].y + fOffsetY;
